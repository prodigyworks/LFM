...
function GetRefereeName($id) {
	$qry = "SELECT A.name FROM {$_SESSION['DB_PREFIX']}referee A " .
			"WHERE A.id = $id ";
	$result = mysql_query($qry);
	$name = "Unknown";
...
function sendRoleMessage($role, $subject, $message, $attachments = array()) {
	...
	if($result) {
		while (($member = mysql_fetch_assoc($result))) {
			smtpmailer($member['email'], $_SESSION['EMAIL_ADDRESS'], $_SESSION['EMAIL_NAME'], $subject, getEmailHeader() . "<h4>Dear " . $member['firstname'] . ",</h4><p>" . $message . "</p>" . getEmailFooter(), $attachments);
...
function sendTeamMessage($id, $subject, $message, $footer = "", $attachments = array()) {
	...
	if($result) {
		while (($member = mysql_fetch_assoc($result))) {
			smtpmailer($member['email'], 'confirmation@tclplanner.co.uk', $_SESSION['EMAIL_NAME'], $subject, getEmailHeader() . "<h4>Dear " . $member['firstname'] . ",</h4><p>" . $message . "</p>" . getEmailFooter(). $footer, $attachments);
...
function sendUserMessage($id, $subject, $message, $footer = "", $attachments = array(), $action = "") {
	...
	if($result) {
		while (($member = mysql_fetch_assoc($result))) {
			smtpmailer($member['email'], $_SESSION['EMAIL_ADDRESS'], $_SESSION['EMAIL_NAME'], $subject, getEmailHeader() . "<h4>Dear " . $member['firstname'] . ",</h4><p>" . $message . "</p>" . getEmailFooter(). $footer, $attachments);
...
function getLoggedOnTeamID() {
	...
	if (! isset($_SESSION['SESS_TEAM_ID'])) {
		return 0;
	}
